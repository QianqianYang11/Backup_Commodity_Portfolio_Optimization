...
# output_excel_path = 'D:/2023semester/Lund University/thesis/new_uni_garch.xlsx'
...
# dcc_corr.to_excel('D:/2023semester/Lund University/thesis/new_dcc_garch.xlsx')
...
# matrix_excel_path = 'D:/2023semester/Lund University/thesis/new_average_dcc_matrix.xlsx'
...
with pd.ExcelWriter('D:/2023semester/Lund University/thesis/new_uni_garch.xlsx') as writer:
...
hessian_approx = numerical_hessian(lambda x: loglike_skewt_dcc(x, udata_list), opt_out.x)
...
def loglike_skewt_dcc_wrapper(theta):
    return loglike_skewt_dcc(theta, udata_list)
...
N = len(udata_list[0])  
hessian_approx = estimate_hessian(loglike_skewt_dcc_wrapper, opt_out.x)
var_cov_matrix = inv(hessian_approx)
standard_errors = np.sqrt(np.diag(var_cov_matrix))
t_stats = opt_out.x / standard_errors
...
results_df.to_excel('D:/2023semester/Lund University/thesis/new_test_dcc.xlsx')

p_values = []
for t_stat in t_stats:
    if np.abs(t_stat) > 10:  
        p_value = 2 * (1 - t.cdf(10, df=N - len(opt_out.x))) 
    else:
