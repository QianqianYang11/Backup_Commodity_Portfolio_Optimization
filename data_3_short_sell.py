...
#covariance
qt_data = pd.read_csv('D:/2023semester/Lund University/thesis/vech_t_cov.csv')
cov_matrices = []
...
print("Qt:", Qt.shape) 
#correlation
rt_data = pd.read_csv('D:/2023semester/Lund University/thesis/vech_t_cor.csv')
cor_matrices = []
...
constraints = [cp.sum(w) == 1]  # Removed non-negativity constraint
problem = cp.Problem(objective, constraints)
problem.solve(solver=cp.SCS)
optimal_weights_min_correlation = w.value

# Time-varying Minimum Correlation
...
min_correlation_weight = pd.DataFrame(optimal_weights_time_varying_min_correlation.T, index=dates, columns=commodity_names)
min_correlation_weight.to_excel('D:/2023semester/Lund University/thesis/optimization_min_correlation.xlsx')
...
# Box constraints
lower_bound = -1  # Allow short selling up to 100%
upper_bound = 1  # Allow long positions up to 100%
...
for i in range(n_days - window_size + 1):
    ...
    expected_return = cp.sum(cp.multiply(np.mean(historical_window, axis=0), w))  # Calculate expected return of the portfolio
    regularization_term = cp.norm(w, 2) * 1e-4  # Add a small regularization term
    objective = cp.Minimize(gamma * portfolio_CVaR - (1 - gamma) * expected_return + regularization_term)
    constraints = [cp.sum(w) == 1, w >= lower_bound, w <= upper_bound]  # Add box constraints
    ...
    try:
        ...
        if problem.status not in ["infeasible", "unbounded"]:
            ...
            print(f"Optimization issue at time {i}: {problem.status}")
            optimal_weights_mean_CVaR[i] = np.nan  # Use nan to indicate failed optimization
    except Exception as e:
        print(f"Exception at time {i}: {str(e)}")
        optimal_weights_mean_CVaR[i] = np.nan
...
mean_CVaR_weight.to_excel('D:/2023semester/Lund University/thesis/optimization_mean_CVaR.xlsx')
...
combined_df = pd.concat([average_weights_df, stats_df], axis=0)
output_excel_path_combined = 'D:/2023semester/Lund University/thesis/aaaa.xlsx'

combined_df.to_excel(output_excel_path_combined, float_format='%.6f')
print("Successful: aaaa combine")
#######################################################
#######################################################
...
for i in range(len(data) - window_size + 1):
    ...
    for name, weight_df in weights.items():
        ...
        sorted_returns = np.sort(returns)
        cutoff_index = int(np.floor(0.05 * len(sorted_returns)))
        cvar = np.mean(sorted_returns[:cutoff_index])
...
output_filepath = 'D:/2023semester/Lund University/thesis/rolling_portfolio_performance.xlsx'
...
line_width = 2
metrics = ['Average Return', 'Variance', 'Sharpe Ratio', 'CVaR']
...
def plot_metrics(rolling_stats_df_pivot, metrics, titles, colors, markers):
    fig, axes = plt.subplots(nrows=len(metrics), ncols=1, figsize=(15, 10), sharex=True)
    ...
    for i, metric in enumerate(metrics):
        ...
        if metric in ['Average Return', 'Variance']:
            ax.yaxis.set_major_formatter(FuncFormatter(percentage_formatter))
        
        if metric in ['Sharpe Ratio', 'CVaR']:
...
# Assuming rolling_stats_df_pivot is already defined
