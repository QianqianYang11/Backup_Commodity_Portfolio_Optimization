...
new_order = ['Equal Weight', 'Mean-Variance', 'Min-Variance', 'Min-Correlation', 'Sharpe Ratio', 'Sortino Ratio', 'Mean-CVaR']
...
titles = ['Equal Weight', 'Mean-Variance', 'Min Variance', 'Sharpe Ratio', 'Sortino Ratio', 'Min Correlation', 'Min CVaR']
...
#####################################################
#####################################################
fig, axes = plt.subplots(nrows=4, ncols=1, figsize=(15, 8), sharex=True)
portfolios = ['Equal Weight', 'Mean-Variance', 'Min Variance', 'Min Correlation']
weights_data = [equal_weight_weight, mean_variance_weight, min_variance_weight, min_correlation_weight]
for i, (portfolio, weights) in enumerate(zip(portfolios, weights_data)):
    ax = axes[i]
    ax.plot(weights.index, weights['Metal'], label='Metal', color='royalblue', marker='o', linestyle='-')
    ax.plot(weights.index, weights['Energy'], label='Energy', color='green', marker='s', linestyle='-')
    ax.plot(weights.index, weights['Agricultural'], label='Agricultural', color='orange', marker='x', linestyle='-')
    ax.set_title(f'{portfolio}', fontsize=18)
    ax.tick_params(axis='y', labelsize=10)
    ax.tick_params(axis='x', rotation=45, labelsize=10)
...
portfolios = ['Sharpe Ratio', 'Sortino Ratio', 'Min CVaR']
weights_data = [Sharpe_Ratio_weight, Sortino_Ratio_weight, mean_CVaR_weight]
for i, (portfolio, weights) in enumerate(zip(portfolios, weights_data)):
    ax = axes[i]
    ax.plot(weights.index, weights['Metal'], label='Metal', color='royalblue', marker='o', linestyle='-')
    ax.plot(weights.index, weights['Energy'], label='Energy', color='green', marker='s', linestyle='-')
